...
-- *.inc files will not get bitbake syntax highlighting if vim-bitbake if loaded before vim-polygot.
-- However, setting vim-polyglot as a dependency of vim-bitbake increases the startup time quite a lot.
-- Therefore, set the filetype manually.
vim.api.nvim_create_autocmd({ "BufNewFile", "BufWinEnter" }, {
    pattern = "*.inc",
    command = "set ft=bitbake",
})
